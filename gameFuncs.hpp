...
std::vector<target> initializeLevel(std::vector<target>& targets, int level)
{
	...
	if (level > 0 && level < 21)
	{
		...
		if (level == 1)
		{
				...
				templates.emplace_back(RECT_ONE);
				templates.emplace_back(RECT_TWO);
				templates.emplace_back(RECT_THREE);
				templates.emplace_back(RECT_FOUR);
		}
		else if (level == 2)
		{
		...
		else if (level == 11 || level == 16)	// DIRECTION_ANGLE
		{
			...
			templates.emplace_back(DIRECTION_ANGLE, randomAngle(), randomSpeed(), RECT_ONE);
			templates.emplace_back(DIRECTION_ANGLE, randomAngle(), randomSpeed(), RECT_TWO);
			templates.emplace_back(DIRECTION_ANGLE, randomAngle(), randomSpeed(), RECT_THREE);
			templates.emplace_back(DIRECTION_ANGLE, randomAngle(), randomSpeed(), RECT_FOUR);
		}
		else if (level == 12)	// DIRECTION_SIN (up or down)
		{
			spots = generateSpots(4 * (12 + level));

			templates.emplace_back(DIRECTION_SIN, ((rand() % 2) == 0) ? PI/2 : 3*PI/2, randomSpeed(), RECT_ONE);
			templates.emplace_back(DIRECTION_SIN, ((rand() % 2) == 0) ? PI/2 : 3*PI/2, randomSpeed(), RECT_TWO);
			templates.emplace_back(DIRECTION_SIN, ((rand() % 2) == 0) ? PI/2 : 3*PI/2, randomSpeed(), RECT_THREE);
			templates.emplace_back(DIRECTION_SIN, ((rand() % 2) == 0) ? PI/2 : 3*PI/2, randomSpeed(), RECT_FOUR);
		}
		else if (level == 14)	// DIRECTION_ANGLE_W_BOUNCE
		{
			spots = generateSpots(4 * (12 + level));

			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, 0, randomSpeed(), RECT_ONE);
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, 0, randomSpeed(), RECT_TWO);
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, 0, randomSpeed(), RECT_THREE);
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, 0, randomSpeed(), RECT_FOUR);
		...
		else if (level == 19)	// DIRECTION_SIN
		{
			...
			templates.emplace_back(DIRECTION_SIN, randomAngle(), randomSpeed(), RECT_ONE);
			templates.emplace_back(DIRECTION_SIN, randomAngle(), randomSpeed(), RECT_TWO);
			templates.emplace_back(DIRECTION_SIN, randomAngle(), randomSpeed(), RECT_THREE);
			templates.emplace_back(DIRECTION_SIN, randomAngle(), randomSpeed(), RECT_FOUR);
		}
	}
	else if (level >= 21)
	{
		...
		// DIRECTION_ANGLE_W_BOUNCE
			...
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, randomAngle(), randomSpeed(), RECT_ONE);
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, randomAngle(), randomSpeed(), RECT_TWO);
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, randomAngle(), randomSpeed(), RECT_THREE);
			templates.emplace_back(DIRECTION_ANGLE_W_BOUNCE, randomAngle(), randomSpeed(), RECT_FOUR);
		}
		/*
...
void DrawTarget(target t, Texture2D facesSmallTexture)
{
	...
	// use faces.png
	if (t.isMenu())
		DrawTextureRec(facesSmallTexture, criminalPosterRects[t.getSpriteRect()], t.getPos(), BLACK);
	else
		DrawTextureRec(facesSmallTexture, criminalPosterRects[t.getSpriteRect()], t.getPos(), WHITE);
	#else
	// dev textures
	Color col;
	if (t.getSpriteRect() == 0) // RECT_ONE
		col = LIME;
	else if (t.getSpriteRect() == 1) // RECT_TWO
		col = RED;
	else if (t.getSpriteRect() == 2) // RECT_THREE
		col = ORANGE;
